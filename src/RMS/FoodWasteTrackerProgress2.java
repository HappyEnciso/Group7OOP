...
public class FoodWasteTrackerProgress2 extends JFrame {
    ...
    FoodWasteTrackerProgress2(){
        ...
        btnUpdate.addActionListener(new ActionListener(){
            ...
            public void actionPerformed(ActionEvent e){
                ...
                dishItemsMap.put(selectedMenuDishes, numberofitem); 

                txtfldItems.setText(""); 
            }
        });
        
        dishItemsMap = new HashMap<>();
        
        
        
        //Event for dish selection
        cmbMenuDishes.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                String selectedDish = cmbMenuDishes.getSelectedItem().toString();
                if (dishItemsMap.containsKey(selectedDish)) {
                    int numberOfItems = dishItemsMap.get(selectedDish);
                    txtfldItems.setText(Integer.toString(numberOfItems));
                } else {
                    txtfldItems.setText("");
                }
            }
    ...
    public void menuOption(String menuCategory){
        ...
        //Set the previously entered value if available
        String selectedDish = cmbMenuDishes.getSelectedItem().toString();
        if (dishItemsMap.containsKey(selectedDish)) {
            int numberofitem = dishItemsMap.get(selectedDish);
            txtfldItems.setText(Integer.toString(numberofitem));
        } else {
            txtfldItems.setText("");
        }
    }
